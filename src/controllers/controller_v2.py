...
class ControllerV2:
    ...
    def __init__(
        self, resource_name, base_route, endpoint_suffix, templates_dir, app, handler
    ):
        """Constructor
        ...
        """
        self.resource_name = (
            i18n("interface.common.%s" % endpoint_suffix) or resource_name
        )
    ...
    def add_routes(self, app):
        ...
        # index
        app.add_url_rule("/%s" % base_route, base_route, self.index, methods=["GET"])
        # new
        app.add_url_rule(
        ...
        # create
        app.add_url_rule(
            "/%s" % base_route, "create_%s" % suffix, self.create, methods=["POST"]
        )
        # edit
        app.add_url_rule(
            "/%s/<int:id>/edit" % base_route,
            "edit_%s" % suffix,
            self.edit,
            methods=["GET"],
        )
    ...
    def setup_models(self):
        ...
        self.config_models = ConfigModels(
            db_engine,
            config_handler.conn_str(),
            [
                "pbms_quan_ly_tham_so",
                "pbms_quan_ly_nhom_tham_so",
                "pbms_quan_ly_phan_loai_tai_san",
                "pbms_quan_ly_don_vi",
                "pbms_dm_tinh_thanh",
                "pbms_dm_quan_huyen",
                "pbms_dm_phuong_xa",
        ...
        self.RegistrationRequest = self.config_models.model("registration_requests")
        ...
        self.PBMSQuanLyPhanLoaiTaiSan = self.config_models.model(
            "pbms_quan_ly_phan_loai_tai_san"
        )
        self.PBMSQuanLyDonVi = self.config_models.model("pbms_quan_ly_don_vi")
    ...
    def create(self):
        ...
        if form.validate_on_submit():
            try:
                ...
                flash(
                    i18n(
                        "interface.main.new_resource_message_success",
                        [self.resource_name],
                    ),
                    "success",
                )
                ...
                flash(
                    i18n(
                        "interface.main.new_resource_message_error",
                        [self.resource_name],
                    ),
                    "warning",
                )
    ...
    def update_form_collection(
        ...
        query = session.query(relation_model).order_by(
            getattr(relation_model, name_attr)
        )
        items = query.all()
    ...
    def update_collection(
        self, collection, multi_select, relation_model, id_attr, session
    ):
    ...
    def pagination_args(self, session_params={}):
        ...
        per_page = self.to_int(
            ...
            session_params.get("per_page", self.DEFAULT_PER_PAGE),
            1,
        )
